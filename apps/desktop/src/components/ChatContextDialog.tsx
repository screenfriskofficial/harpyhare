...
function libraryGroups(library: ContextLibrary, query: string, rootName: string): DocGroup[] {
  const needle = query.trim().toLowerCase();
  const matches = (doc: ContextDoc) => needle === "" || doc.name.toLowerCase().includes(needle);
  return [
    {
      id: "",
      name: library.folders.length > 0 ? rootName : "",
...
function LibraryPicker({
  ...
  const groups = libraryGroups(library, query, t("common.noFolder"));
  const selected = new Set(selectedDocIds);

  if (library.docs.length === 0) {
    return (
      <p className="text-caption text-muted-foreground">{t("hud.contextDialog.libraryEmpty")}</p>
    );
  }
    ...
    <div className="flex min-h-0 flex-col gap-1.5">
      {library.docs.length >= LIBRARY_FILTER_MIN_DOCS && (
        <Input
          ...
          placeholder={t("hud.contextDialog.filterPlaceholder")}
          aria-label={t("hud.contextDialog.filterPlaceholder")}
        />
      )}
      {groups.length === 0 ? (
        <p className="text-caption text-muted-foreground">{t("common.nothingFound")}</p>
      ) : (
  ...
  return (
    <>
      <div className="flex min-h-0 flex-1 flex-col gap-3.5 overflow-y-auto">
        <div className="flex min-h-0 flex-col gap-1.5">
          <SectionLabel>
            {t("hud.contextDialog.fromLibrary")}
            {selectedDocIds.length > 0 && (
              <span className="ml-1.5 text-muted-foreground tabular-nums">
                {t("hud.contextDialog.selected", { count: selectedDocIds.length })}
              </span>
            )}
          </SectionLabel>
          <LibraryPicker
        ...
        <div className="flex flex-col gap-1.5">
          <SectionLabel>{t("hud.contextDialog.ownText")}</SectionLabel>
          <p className="text-caption text-muted-foreground">{t("hud.contextDialog.ownTextHint")}</p>
          <Textarea
            rows={6}
            value={draft}
            onChange={(e) => {
